...
type Token struct {
	...
	TokenType    Type      `json:"token_type"`              // 令牌的类型 类型包含: bearer/jwt  (默认为bearer)
	GrantType    GrantType `json:"grant_type"`              // 授权的类型
	...
	Scope       string `json:"scope"`                 // 令牌的作用范围: detail https://tools.ietf.org/html/rfc6749#section-3.3
	CreatedAt   int64  `json:"create_at"`             // 凭证创建时间
	ExpiresIn   int64  `json:"expires_in"`            // 凭证过期的时间

	IsSystemAdmin     bool     `json:"is_system_admin,omitempty"` // 是否是系统管理员
	IsDomainAdmin     bool     `json:"is_domain_admin,omitempty"` // 是否是域管理员
	AvaliableProjects []string `json:"available_projects"`        // 该用户可以访问的项目列表
	Roles             []string `json:"roles"`                     // 该凭证的权限列表
}
