...
func CreateProject(w http.ResponseWriter, r *http.Request) {
	...
	// 交给业务控制层处理
	if err := global.Store.CreateProject(p); err != nil {
		response.Failed(w, err)
		return
	}
...
// ListDomainProjects list users under his domain
func ListDomainProjects(w http.ResponseWriter, r *http.Request) {
	var err error
...
// ListUserProjects list users under his domain
func ListUserProjects(w http.ResponseWriter, r *http.Request) {
	tk := context.GetTokenFromContext(r)

	projects, err := global.Store.ListUserProjects(tk.DomainID, tk.UserID)
	if err != nil {
		response.Failed(w, err)
		return
	}
